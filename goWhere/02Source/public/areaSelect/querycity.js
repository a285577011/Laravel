(function($){
    $.querycity = function(input,options){
	    ...
	    popContainer.find('a').on('click',function(){
		    input.val($(this).html());
		    $('#'+options.inputCityIdName).val($(this).attr('data-city'));
		    popMain.hide();
	    });
        ...
        input.blur(function(){
            ...
            if(13 == keyCode){
                if(suggestMain.find('.list_city_container a.selected').length > 0){
                    ...
                    $('#'+options.inputCityIdName).val(suggestMain.find('.list_city_container a.selected').attr('data-city'));
                    suggestMain.hide();
                }
            }
        ...
        $(document).on('click','#suggest_city_'+input.attr('id')+' .list_city_container a',function(){
            input.val($(this).children('b').text());
            $('#'+options.inputCityIdName).val($(this).attr('data-city'));
            suggestMain.hide();
        });
        $(document).on('mouseover','#suggest_city_'+input.attr('id')+' .list_city_container a',function(){
        	t_suggest_focus = true; 
        });
        $(document).on('mouseout','#suggest_city_'+input.attr('id')+' .list_city_container a',function(){
        	t_suggest_focus = false;
        });
        $(document).on('mouseover','#suggest_city_'+input.attr('id')+' .page_break a',function(event){
            t_suggest_focus = true;        
        });
        $(document).on('mouseout','#suggest_city_'+input.attr('id')+' .page_break a',function(event){
            t_suggest_focus = false;        
        });
        $(document).on('click','#suggest_city_'+input.attr('id')+' .page_break a',function(event){
    	...
    	function loadCity(){
	    	...
	    	for(var item in hotList){
			    ...
			    var _data = options.data[hotList[item]];
                        if(_data) {
                            cityList.append("<a href='javascript:void(0)' ><span>"+_data[1]+"</span><b>"+_data[0]+"</b></a>");   
                        }
		    }		
    	...
    	function queryCity(){
            ...
            for(var item in options.data){			
                ...
                if(typeof (_data) != 'undefined'){
                    if(_data[1].indexOf(value) >=0 || _data[0].indexOf(value) >=0 ){					                   
                        isHave = true;
                        _tmp.push(_data);
                    }
                }
...
;
		    if(isHave){
                ...
                var _data= _tmp[item];
                city_container.append("<a href='javascript:void(0)' style='display:none' data-city="+_data[2]+"><span>"+_data[1]+"</span><b>"+     _data[0] +"</b></a>");                
                }
			    suggestMain.find('.list_city_head').html(value+","+options.pingyinOrder);
                setAddPage(1);
                setTopSelect()
    		}else{
	    		suggestMain.find('.list_city_head').html("<span class='msg'>"+options.nofundText+value+"</span>");
		    }
            suggestMain.show();
	    }
        ...
        function popInit(){
            ...
            if(!options.tabs){
                ...
                labelMain.remove();
                for( var item in options.data){
                      $("#label_"+input.attr('id')+"_container").append("<li><a href='javascript:void(0)' data-city="+_data[2]+">"+options.data[item][0]+"</a></li>");
                }
                return;
            }
    		for(var itemLabel in options.tabs){
			    ...
			    for(var item in options.tabs[itemLabel]){
				    ...
				    $("#label_"+input.attr('id')+index+"_container").append("<li><a href='javascript:void(0)' data-city="+options.data[cityCode][2]+">"+options.data[cityCode][0]+"</a></li>");
			    }
		    }			
        }
